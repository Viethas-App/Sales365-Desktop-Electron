"use strict";
(self["webpackChunkSales365_Viethas"] = self["webpackChunkSales365_Viethas"] || []).push([["src_app_cafe_desktop_menu_management_stock_inventory_inventory_module_ts"],{

/***/ 37461:
/*!*************************************************************************************!*\
  !*** ./src/app/cafe/desktop/menu/management/stock/inventory/inventory.component.ts ***!
  \*************************************************************************************/
...
class InventoryComponent {
    constructor(router, vhQueryCafe, vhComponent, languageService, vhAuth, nzModalService) {
        this.router = router;
    ...
    getData() {
        this.vhQueryCafe.getInventorytimes().then((localInventorytimes) => {
            ...
            this.inventory_times = localInventorytimes.filter(item => item.id_branch == this.vhQueryCafe.getDefaultBranch()._id).reverse();
            console.log(this.inventory_times);
        });
    }
    goBack() {
    ...
    addInventory() {
        if (this.name_inventory.trim().length) {
            ...
            this.vhComponent.showLoading("", "transparent-loading").then(() => {
                this.vhQueryCafe.addInventorytime(data).then(res => {
    ...
    editInventory() {
        if (this.name_inventory.trim().length) {
            this.vhComponent.showLoading("", "transparent-loading").then(() => {
                this.vhQueryCafe.updateInventorytime(this.id_choose._id, { name: this.name_inventory }).then((res) => {
                    this.vhComponent.hideLoading(0).then(() => {
                        this.vhComponent.showToast(2000, this.languageService.translate("Updated successfully"), "success-toast");
                        this.visible_edit = false;
                        this.id_choose.name = this.name_inventory;
                        // this.name_inventory = "";
                        // this.getData();
                    });
                });
    ...
    deleteInventoryTimes() {
        if (this.inventory_times.balanced) {
            ...
            this.vhComponent.showLoading("", "transparent-loading").then(() => {
                this.vhQueryCafe.deleteInventorytime(this.id_choose).then((res) => {
                    this.vhComponent.hideLoading(0).then(() => {
                        if (res) {
                            ...
                            this.isVisible = false;
                            this.inventory_times = this.inventory_times.filter(item => item._id != this.id_choose);
                            // this.getData()
                        }
                        else {
    ...
    gotoDetail(item) {
        this.router.navigate(["/cafe/dashboard/management/stock/inventory/detail"], { state: { inventory_times: item } });
    }
}
...
/***/ 71915:
/*!**********************************************************************************!*\
  !*** ./src/app/cafe/desktop/menu/management/stock/inventory/inventory.module.ts ***!
  \**********************************************************************************/
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
...
/* harmony import */ var _inventory_component__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./inventory.component */ 37461);
...
//# sourceMappingURL=src_app_cafe_desktop_menu_management_stock_inventory_inventory_module_ts.js.map